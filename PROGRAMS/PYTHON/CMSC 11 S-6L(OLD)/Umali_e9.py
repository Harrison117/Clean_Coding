...
def saveData(info): ## Turns dictionary into a text file...
    ...
    if n=="1":
        new_file=open("studrec.txt","a")
        new_file.write(str(info)+"\n") 
        ## dictionary can't be stored in the text file, so it has to be converted into a string
        ## add new line to make way for updated data
        new_file.close()
        divider()
