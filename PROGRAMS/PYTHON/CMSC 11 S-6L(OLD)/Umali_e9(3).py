...
def deleteStudent(info,student_name):
    if student_name in student_record:
        ...
        divider()
        print(student_record.pop(student_name,"Student does not exist...")) ## Always prints "Name does not exist" if name does not exist
        divider()
        input("Press enter to continue...")
...
def loadData(info):
    try: 
        ...
        ## The type of error when the asked file doesn't exist; this executes when the error should happen
        divider()
